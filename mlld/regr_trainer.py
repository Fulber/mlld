...
class Regression(object):
	...
	def tune_parameters(self, file):
		...
		clf = GridSearchCV(LogisticRegression(class_weight = 'balanced'), parameters, cv = 2, scoring = make_scorer(precision_score, pos_label = 1))
		clf.fit(fT, lT)
...
def main(argv):
	...
	my_trainer = Regression(-1, debug = debug, optimise = optimise, exp = exp)
	if tune:
		my_trainer.tune_parameters(file)
	else:
		if proba:
			print(my_trainer.validate_proba(file, 0.872))
		else:
